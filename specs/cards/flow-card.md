...
- **Edges**: orthogonal (Manhattan-style) path with soft curved corners, not full right angles. Each end anchors to whichever side (N/S/E/W) of the node faces the other node — picked automatically from relative `(col, row)` position, not configurable. For diagonal-ish node pairs (within ~2x ratio of row/column distance), an edge routes via the less-congested axis at each node instead of always the dominant one — e.g. if a node already has an edge leaving East, a second edge to a roughly-diagonal target leaves via North/South instead of also using East. Edges that still end up sharing the exact same `(node, side)` anchor (no viable alternate axis) fan out tangentially along that side so they don't overlap. Active = dashed (12px/8px), animated 0.8s CSS keyframe loop, colored per `edge.color`. Inactive = solid gray `#444`, no animation.
- **Edge active logic**:
